...
def upgrade() -> None:
    # На всякий случай сносим старые типы, если они остались в БД
    op.execute("DROP TYPE IF EXISTS migrationcardstatus CASCADE")
    op.execute("DROP TYPE IF EXISTS migrationevent CASCADE")

    # ВАЖНО:
    # Создаём ENUM-тип из .value Python Enum'ов,
    # чтобы в БД значения были 'active', 'submitted_for_extension', ...
    migration_card_status = sa.Enum(
        *[status.value for status in MigrationCardStatus],
        name="migrationcardstatus",
    )
    migration_event = sa.Enum(
        *[event.value for event in MigrationEvent],
        name="migrationevent",
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "status",
            migration_card_status,
            nullable=False,
            # DEFAULT 'active' — это значение .value из Enum
            server_default=MigrationCardStatus.ACTIVE.value,
        ),
        sa.Column(
            "paused_by_user",
            sa.Boolean(),
